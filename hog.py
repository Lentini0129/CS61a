...
def boar_brawl(player_score, opponent_score):
    ...
    score = abs(opponent_tens - player_ones) * 3
    return max(score, 1)
    # END PROBLEM 2
...
def simple_update(num_rolls, score, opponent_score, dice=six_sided):
    turn_score = take_turn(num_rolls, score, opponent_score, dice)
    score += turn_score
    return score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opponent_score in range(goal):
            if strategy(score, opponent_score) != num_dice:
                return False
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if sus_points(score + take_turn(num_rolls, score, opponent_score)) >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
